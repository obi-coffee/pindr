-- Loop Phase D: announce_arrival RPC. "i'm here" on the day-of screen.
-- No arrivals table in v1 — the ping IS the product. Validates that the
-- caller is actually in this locked round and that it's round day, then
-- fans out to the push-on-arrival Edge Function via notify_push.
--
-- Window: from 12 hours before tee time to 6 hours after. Outside that,
-- the button shouldn't even render client-side; the server check is the
-- backstop.
