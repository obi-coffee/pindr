-- Loop Phase E: availability. Same iteration-friendly JSONB pattern as
-- profile_answers — keys are slot ids from lib/profile/availability.ts,
-- values are true. Empty '{}' means "not set". Changing a slot id drops
-- prior answers, so be deliberate (same caveat as question ids).

alter table public.profiles
  add column if not exists availability jsonb not null default '{}'::jsonb;

-- get_profile_by_id grows an availability column (appended last) so the
-- full profile view can render it. Column order otherwise preserved from
-- 20260502100000. RETURNS TABLE shape changes require drop+recreate.
